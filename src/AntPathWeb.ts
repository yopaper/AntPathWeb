...
import { CreateTestMap } from "./Debug.js";
import { AntNest } from "./AntNest.js";

Canvas.Init();
GameMap.Init();

const MapSize:Type.Vector2={X:99, Y:99};
const MapCenter:Type.Vector2 = {X:Math.floor(MapSize.X/2), Y:Math.floor(MapSize.Y/2)};

for(var x=0; x<=MapSize.X ;x++){
    for(var y=0; y<=MapSize.Y; y++){
        var DeltaToCenter:Type.Vector2 = {X:Math.abs(x-MapCenter.X), Y:Math.abs(y-MapCenter.Y)};
